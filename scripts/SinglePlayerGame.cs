...
$InvBanList[SinglePlayer, "MiningTool"] = 1;
...
function setSinglePlayerGlobals() 
{
   ...
   //Has been totally modified to support the functioning of multiple Campaigns
	$playerTeam 				= $Training::PlayerTeam[$CurrentMission];
	$teamSkin[$playerTeam] 		= addTaggedString($Training::Skin[$CurrentMission]);
	$teamName[$playerTeam] 		= addTaggedString($Training::TeamName[$CurrentMission]);
	$holoName[$playerTeam] 		= stripChars($Training::TeamName[$CurrentMission]," ");
	$switchSkin[$playerTeam] 	= addTaggedString($Training::Skin[$CurrentMission]);
	$playerLivesAtEasy			= $Training::StartLives[$CurrentMission];
	
	$EnemyTeam 					= $Training::EnemyTeam[$CurrentMission];
	$teamSkin[$EnemyTeam] 		= addTaggedString($Training::EnemySkin[$CurrentMission]);
	$teamName[$EnemyTeam] 		= addTaggedString($Training::EnemyName[$currentMission]);
...
function addEnemies()
{
	...
	for(%i = 0; %i < %num; %i++){
		...
		%client = AIConnect(%name, $EnemyTeam, $missionBotSkill[$pref::TrainingDifficulty], true, %voice, %voicePitch);
 		$Enemy[%i] = %client;
 		%client.race = $Training::EnemyRace[$CurrentMission];
...
function spawnWave(%wave)
{
	for(%i=1; %i<=$numberInWave[$pref::TrainingDifficulty]; %i++) {
		...
		%thisAi.race = $Training::EnemyRace[$CurrentMission];
...
function addPlayersTeam(%num)
{
	...
	for(%i=0; %i< %num; %i++){
        ...
        %client.race = $Training::Race[$CurrentMission];
...
function SinglePlayerGame::clientMissionDropReady(%game, %client)
{
	...
	$Player = %client;
	$player.race = $Training::Race[$CurrentMission];
    $player.sex = $Training::Sex[$CurrentMission];
	$player.team = $Training::PlayerTeam[$CurrentMission];
	$player.setTeam($Training::PlayerTeam);
