...
////////////////////////////////////////////////////////////////////////////////
function stationTrigger::onEnterTrigger(%data, %obj, %colObj) //Did a shitty RPG hack to ask if the player wants to pay before actually running the inv..
{
   ...
   if(%colObj.getDataBlock().className !$= "Armor" || %colObj.getState() $= "Dead")
      ...
      %colObj.client.isAtInv = true;
      %colObj.obj = %obj;
      %colObj.data = %data;

   if ($CurrentMissionType $= "RPG")
   {
    if (%colObj.client.money > 100)
    commandToClient(%colObj.client,'HandleScriptedCommand',1,"Refill","To refill your inventory, you must pay $100.","commandToServer('DoRefill');");
    else
    messageClient(%colObj.client,'msgNotEnoughCashForRefill',"\c3You do not have enough money to refill your inventory.");
   }
   else
   serverCmdDoRefill(%colObj.client);
 return;
}
...
function stationTrigger::onLeaveTrigger(%data, %obj, %colObj)
{
   ...
   commandToClient(%colObj.client,'HandleScriptedCommand',0,"MessageBoxYesNoDLG");
   if(%obj.station)
   {
...
{
   %vel = getWords(%colObj.getVelocity(), 0, 1) @ " 0";
   if((VectorLen(%vel) < 28) && (%obj.triggeredBy != %colObj)) // z0dd - ZOD, 4/27/02. Can engage deployed inv at higher speeds. Was 22. Now ~100kph
   {
      // -------------------------------------------------
