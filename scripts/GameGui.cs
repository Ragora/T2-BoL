...
function JoinGame(%address)
{
   ...
   //Make sure the server is T2BOL. If not, join as a derm if we're Draakan or Creole
   if (getField(GMJ_Browser.getServerInfoString(),2) !$= "T2BOL")
   {
    %race = getWord(%playerRaceGender,0);
    if (%race $= "Draakan" || %race $= "Criollos" )
    %playerRaceGender = "Bioderm";
   }
   ...
   if ( $Host::MissionType !$= "" )
   {
      ...
      if ( %type != $HostTypeCount )
      {
         ...
         GMH_MissionType.onSelect( %type, "" );
           ...
           if ($Host::MissionType $= "RPG" || $Host::MissionType $= "SV")
           {
           GMH_BotsEnabledTgl.setValue(0);
           GMH_BotsEnabledTgl.onAction();
           GMH_BotsEnabledTgl.setActive(0);
           }
...
function GMH_MissionType::onSelect( %this, %id, %text )
{
   ...
   // ---------------------------------------------------
   // Disable the 'enable AI' button if "RPG" is selected..
   if (%text $= "Role Playing" || %text $= "Survival") //Or Survival..
   {
   GMH_BotsEnabledTgl.setValue(0);
   GMH_BotsEnabledTgl.onAction();
   GMH_BotsEnabledTgl.setActive(0);
   ...
   GMH_BotsEnabledTgl.setValue($Host::BotsEnabled);
   GMH_BotsEnabledTgl.onAction();
   GMH_BotsEnabledTgl.setActive(1);
   }
}
   ...
   switch ( GMW_RaceGenderPopup.getSelected() )
   {
      case 1: 
         if ( %armor $= "heavy" )
            ...
            %shape = %armor @ "_female";
      case 2 or 3 or 4 or 5 or 6: %shape = "bioderm_" @ %armor;
...
function GMW_SkinPopup::fillList( %this, %raceGender )
{
   ...
   for ( %file = findFirstFile( %path @ "*" @ %pattern ); %file !$= ""; %file = findNextFile( %path @ "*" @ %pattern ) )
   {
      ...
      if ( %skin !$= "basebot" && %skin !$= "basebbot" )
      {
         ...
         if ( %customSkins != %baseSkin && !%loaded[%skin] ) //Avoid Duplicate skins cluttering up your selection ^.^
         {
            if ( %baseSkin )
