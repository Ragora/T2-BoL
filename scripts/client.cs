...
function RTS_RightButton(%val)
{
   ...
   else
   {
      ...
      Canvas.cursorOn(); //Show the cursor
   }
}

function clientCmdAlxPlayMusic(%file)
{
alxPlayMusic(%file);
return %file;
}

function RTS_LeftButton(%val)
{
 if (%val) //We clicked -- we must make our little selection box o' doom
 {
  %pos = Canvas.getCursorPos();
  PlayGui.selectBox = new HudZoom() {
  profile = "ZoomHudProfile";
  position =  %pos;
  extent = "5 5"; //Any smaller and you don't know it exists.
  visible = "1";
  fillColor = "0.250000 0.250000 0.250000 0.250000";
  frameColor = "0.000000 1.000000 0.000000 1.000000";
  opacity = "1";
  };
  PlayGui.add(Playgui.selectBox);
  RTS_RightButton(0);
 ...
 {
  if (IsObject(Playgui.selectBox))
  Playgui.selectBox.delete();
  Canvas.setCursor(DefaultCursor);
  $RTSSelect = false;
 }
}

//I should get Emp to handle this.. he knows his vect0r math
function RTS_SelectLoop() //For now, unless there's a function on the Canvas that is called when le' cursor is moved
{
if (!$RTSSelect)
return;
...
PlayGui.selectBox.setExtent(%x * 0.5, %y * 0.5);
schedule(1,0,"RTS_SelectLoop");
}

function clientCmdIsRTSGame()
{
hudClusterBack.opacity = 0; //Make it invisible
clientCmdShowCursor(1); //Show our cursor
}
