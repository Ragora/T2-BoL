...
function AIConnection::onAIConnect(%client, %name, %team, %skill, %offense, %voice, %voicePitch)
{
   ...
   if ($currentMissionType !$= "RPG") //Ok.. the bots should not be added to the lobby
   {
   ...
   //If the gamemode is RPG, add to our AIGroup -- so the bot is not shown in server info
   if ($CurrentMissionType $= "RPG")
   {
    if (!IsObject(AIGroup))
    MissionCleanup.add(new SimGroup(AIGroup));
